...
public class SaveController {
    ...
    public void saveOrder(String identifier, TableView<Iron> tableView, double totalG, double totalR, double totalV, double totalM, String orderData, String orderer, String note){
        ...
        if (System.getProperty("os.name").toLowerCase().contains("win")) {
            ironerPath = "C://Users/Public/RadniNalozi";
        } else {
            ironerPath = userHome + "/Documents/RadniNalozi";
        }
        ...
        try {
            if (file.createNewFile()) {
                exportTableViewToPdf(tableView, file.getAbsolutePath(), totalG, totalR, totalV, totalM, name, orderData, orderer, note);
            } else {
    ...
    private void exportTableViewToPdf(TableView<Iron> tableView, String filePath, double totalG, double totalR, double totalV, double totalM, String orderIdentifier, String orderData, String orderer, String note) {
        try {
            ...
            this.initInfo(document, orderIdentifier, orderer, orderData, note);
            this.initTable(document, tableView);
            this.initTotal(document, totalG, totalR, totalV, totalM);
    ...
    private void initHeader(Document document){
    ...
    private void initInfo(Document document, String orderIdentifier, String orderer, String orderData, String note) {
    ...
    private void initTable(Document document, TableView<Iron> tableView) throws IOException {
    ...
    private void initTotal(Document document, double totalG, double totalR, double totalV, double totalM){
        PdfPTable pdfTotalTable = new PdfPTable(4);
        pdfTotalTable.setWidthPercentage(100);
        pdfTotalTable.setWidths(new float[]{2.0f, 2.0f, 2.0f, 2.0f});
        ...
        PdfPCell headerMreze = new PdfPCell(new Phrase("Total Mreze:"));
        headerMreze.setHorizontalAlignment(Element.ALIGN_LEFT);
        headerMreze.setBackgroundColor(Color.LIGHT_GRAY);

        pdfTotalTable.addCell(headerUzengije);
        pdfTotalTable.addCell(headerSipke);
        pdfTotalTable.addCell(headerVezano);
        pdfTotalTable.addCell(headerMreze);
        ...
        PdfPCell mrezeCell = new PdfPCell(new Phrase(String.format("%.2f", totalM) + " kg"));
        mrezeCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
        pdfTotalTable.addCell(mrezeCell);
